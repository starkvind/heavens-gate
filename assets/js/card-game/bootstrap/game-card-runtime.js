(function () {
    'use strict';
    if (window.__HG_CARD_GAME_RUNTIME_LOADED__) { return; }
    window.__HG_CARD_GAME_RUNTIME_LOADED__ = true;

    var domApi = window.HGCardGame && window.HGCardGame.core ? window.HGCardGame.core.dom : null;
    var eventsApi = window.HGCardGame && window.HGCardGame.core ? window.HGCardGame.core.events : null;
    var appRuntime = window.HGCardGame && window.HGCardGame.bootstrap ? window.HGCardGame.bootstrap.appRuntime : null;
    var packsApi = window.HGCardGame && window.HGCardGame.packs ? window.HGCardGame.packs : null;
    var shopApi = window.HGCardGame && window.HGCardGame.shop ? window.HGCardGame.shop : null;
    var packsDomain = null;
    var shopDomain = null;
    var shopRenderFrame = 0;
    var root = domApi && typeof domApi.findRoot === 'function'
        ? domApi.findRoot()
        : document.querySelector('.hg-cards');
    if (!root) { return; }
    ...
    var stateFactory = window.HGCardGame && window.HGCardGame.core && window.HGCardGame.core.state;
    var state = stateFactory && typeof stateFactory.createLegacyState === 'function'
        ? stateFactory.createLegacyState(root)
        : {
    ...
    var els = domApi && typeof domApi.collectElements === 'function'
        ? domApi.collectElements()
        : {
            ...
            packResultsSection: document.getElementById('hgPackResultsSection'),
    ...
    function updateDesktopHashPanels() {
        ...
        if (state.view === 'gacha') {
            ...
            setDesktopSection(els.packResultsSection || els.packResults, !shopActive && packResultsAvailable());
            setDesktopSection(els.shopSection, shopActive);
            if (shopActive) { scheduleShopRender(); }
        }
    ...
    function createShopState() {
        ...
        return domain ? domain.createShopState() : null;
    }
    ...
    function loadShopState() {
        var domain = getShopDomain();
        return domain ? domain.loadShopState() : null;
    }

    function saveShopState() {
        var domain = getShopDomain();
        if (domain) { domain.saveShopState(); }
    }

    function syncShopState() {
        var domain = getShopDomain();
        return domain ? domain.syncShopState() : { packs: 0, mnemones: 0 };
    }

    function dailyFreePacksRemaining() {
        var domain = getShopDomain();
        return domain ? domain.dailyFreePacksRemaining() : 0;
    }

    function claimDailyFreePacks(amount) {
        var domain = getShopDomain();
        return domain ? domain.claimDailyFreePacks(amount) : false;
    }

    function dailyGiftState() {
        var domain = getShopDomain();
        return domain ? domain.dailyGiftState() : { key: '', claimed: 0 };
    }

    function dailyGiftRemaining() {
        var domain = getShopDomain();
        return domain ? domain.dailyGiftRemaining() : 0;
    }

    function claimDailyGift() {
        var domain = getShopDomain();
        return domain ? domain.claimDailyGift() : false;
    }
    ...
    function normalizeWorkPendingRewards(value) {
        var pending = Math.max(0, clampInt(value, 0));
        return MAX_MNEMONES > 0 ? Math.min(MAX_MNEMONES, pending) : pending;
    }

    function normalizeShopPackPurchases(purchases) {
        var domain = getShopDomain();
        return domain ? domain.normalizeShopPackPurchases(purchases) : {};
    }
    ...
    function syncDailyShopPackPurchases() {
        var domain = getShopDomain();
        return domain ? domain.syncDailyShopPackPurchases() : {};
    }

    function dailyShopPackRemaining(packKind) {
        var domain = getShopDomain();
        return domain ? domain.dailyShopPackRemaining(packKind) : 0;
    }
    ...
    function claimDailyShopPacks(packKind, amount) {
        var domain = getShopDomain();
        return domain ? domain.claimDailyShopPacks(packKind, amount) : false;
    }

    function renderDailyCounter() {
        var domain = getShopDomain();
        if (domain) { domain.renderDailyCounter(); }
    }
    ...
    function normalizeMaterialInventory(inventory) {
        var out = {};
        var materialKeys = Object.keys(UPGRADE_MATERIALS);
    ...
    function limitWorkAssignments(persist) {
        if (!state.collection) { loadCollection(); }
        if (WORK_MAX_ASSIGNMENTS <= 0) { return false; }
    ...
    function packStock(packKind) {
        var domain = getPacksDomain();
        return domain ? domain.packStock(packKind) : 0;
    }

    function packSpace(packKind) {
        var domain = getPacksDomain();
        return domain ? domain.packSpace(packKind) : 0;
    }

    function canOpenPack(packKind) {
        var domain = getPacksDomain();
        return domain ? domain.canOpenPack(packKind) : false;
    }

    function consumePack(packKind) {
        var domain = getPacksDomain();
        if (domain) { domain.consumePack(packKind); }
    }

    function addPack(packKind, amount, options) {
        var domain = getPacksDomain();
        return domain ? domain.addPack(packKind, amount, options) : false;
    }

    function totalPackStock() {
        var domain = getPacksDomain();
        return domain ? domain.totalPackStock() : 0;
    }

    function renderPackInventory() {
        var domain = getPacksDomain();
        if (domain) { domain.renderPackInventory(); }
    }

    function renderShop() {
        var domain = getShopDomain();
        if (domain) { domain.renderShop(); }
    }
    ...
    function normalizeCopyMoveIds(value) {
        ...
        var libraryReady = Object.keys(MOVE_LIBRARY).length > 0;
        var seen = {};
        return value.map(function (entry) {
            return normalizeMoveId(entry, '');
        }).filter(function (id) {
            if (!id || seen[id]) { return false; }
            if (libraryReady && !MOVE_LIBRARY[id]) { return false; }
            seen[id] = true;
            return true;
        }).slice(0, 3);
    }
    ...
    function applyLegacyGameRulesCatalog(legacy) {
    ...
    function gameRulesPayloadReady() {
    ...
    function loadCollection() {
        var loaded = buildLoadedCollectionState(readStoredCollectionData());
        state.collection = normalizeLoadedCollectionState(loaded.collection);
        cleanWorkAssignments(false);
        limitWorkAssignments(false);
        enforceWorkRewardCap(false);
        persistLoadedCollectionState(loaded.shouldPersist);
        return state.collection;
    }

    function readStoredCollectionData() {
        return readMigratedJson(STORAGE_KEY, LEGACY_STORAGE_KEY, null);
    }

    function buildLoadedCollectionState(data) {
        var loaded = {
            collection: null,
            shouldPersist: false
        };
        try {
            loaded.collection = data ? validateCollection(data) : createEmptyCollection();
            loaded.shouldPersist = !!data;
        } catch (e) {
            loaded.collection = createEmptyCollection();
        }
        return loaded;
    }

    function normalizeLoadedCollectionState(collection) {
        collection = collection || createEmptyCollection();
        collection.currency = normalizeCurrency(collection.currency);
        collection.packInventory = normalizePackInventory(collection.packInventory);
        collection.dailyShopPackPurchases = normalizeDailyShopPackPurchases(collection.dailyShopPackPurchases);
        collection.materialInventory = normalizeMaterialInventory(collection.materialInventory);
        collection.workAssignments = normalizeWorkAssignments(collection.workAssignments);
        collection.workPendingRewards = normalizeWorkPendingRewards(collection.workPendingRewards);
        return collection;
    }

    function persistLoadedCollectionState(shouldPersist) {
        if (shouldPersist) { saveCollection(); }
    }

    function renderAfterCollectionLoad() {
    ...
    function saveCollection() {
        ...
        return writeJson(STORAGE_KEY, state.collection);
    }
    ...
    function openPack(packKind, options) {
        var domain = getPacksDomain();
        return domain ? domain.openPack(packKind, options) : [];
    }

    function buyPack(packKind, amount, options) {
        var domain = getShopDomain();
        return domain ? domain.buyPack(packKind, amount, options) : false;
    }

    function buyMaterial(materialKey, amount) {
        var domain = getShopDomain();
        return domain ? domain.buyMaterial(materialKey, amount) : false;
    }

    function claimShopDailyGift(materialKey) {
        var domain = getShopDomain();
        return domain ? domain.claimShopDailyGift(materialKey) : false;
    }

    function buyRemoriaExchange(remoriasAmount) {
        var domain = getShopDomain();
        return domain ? domain.buyRemoriaExchange(remoriasAmount) : false;
    }

    function openAllPacks() {
        var domain = getPacksDomain();
        return domain ? domain.openAllPacks() : [];
    }

    function packResultsAvailable() {
        var section = els.packResultsSection || (els.packResults && els.packResults.closest ? els.packResults.closest('.hg-pack-results') : null);
        return !!(section && section.getAttribute('data-has-pack-results') === '1');
    }

    function setPackResultsAvailable(available) {
        var section = els.packResultsSection || (els.packResults && els.packResults.closest ? els.packResults.closest('.hg-pack-results') : null);
        if (!section) { return; }
        section.setAttribute('data-has-pack-results', available ? '1' : '0');
        section.hidden = !available || (!state.mobile && state.view === 'gacha' && currentHashPanel() === 'shop');
    }
    ...
    function renderPackResults(cards) {
        ...
        setPackResultsAvailable(cards.length > 0);
        ...
        cards.forEach(function (entry, index) {
            fragment.appendChild(renderCard(entry.catalog, entry.instance, { lightweight: true }));
        });
        els.packResults.appendChild(fragment);
    }
    ...
    function migrateCollectionQuality() {
        if (!state.collection || !Array.isArray(state.collection.ownedCards)) { return false; }
        var migrationsApi = window.HGCardGame && window.HGCardGame.data ? window.HGCardGame.data.migrations : null;
        var changed = migrationsApi && typeof migrationsApi.migrateCollectionQualityCopies === 'function'
            ? migrationsApi.migrateCollectionQualityCopies(state.collection.ownedCards, state.catalogById)
            : false;
    ...
    function renderAlbumSlot(card, group) {
        ...
        if (owned) {
            ...
            img.src = card.card_image_url || '/img/og/og_image.webp';
            img.alt = '';
            img.loading = 'lazy';
            img.addEventListener('error', function () { img.src = '/img/og/og_image.webp'; }, { once: true });
    ...
    function combatModesRegistry() {
        return window.HGCardGame && window.HGCardGame.combat ? window.HGCardGame.combat.modes : null;
    }

    function combatModeMeta(mode) {
        var registry = combatModesRegistry();
        return registry && typeof registry.meta === 'function' ? registry.meta(mode) : null;
    }

    function isCombatModeEnabled(mode) {
        var registry = combatModesRegistry();
        if (registry && typeof registry.isEnabled === 'function') {
            return registry.isEnabled(mode);
        }
        return mode === 'daily-boss' || mode === 'training';
    }

    function normalizeCombatMode(mode, fallback) {
        var registry = combatModesRegistry();
        if (registry && typeof registry.normalize === 'function') {
            return registry.normalize(mode, fallback || 'training');
        }
        return mode === 'daily-boss' ? 'daily-boss' : (fallback === 'daily-boss' ? 'daily-boss' : 'training');
    }
        ...
        els.dailyBossSummaries.forEach(function (summary) {
            ...
            summary.innerHTML =
                '<div class="hg-daily-boss-summary__media">' +
                    '<img src="' + escapeHtml(card && card.card_image_url || '/img/og/og_image.webp') + '" alt="">' +
                '</div>' +
    ...
    function updateCombatModeButtons() {
        state.combatMode = normalizeCombatMode(state.combatMode, 'training');
        ...
        els.combatModeButtons.forEach(function (button) {
            ...
            var enabled = isCombatModeEnabled(mode);
            button.classList.toggle('is-active', active);
            button.setAttribute('aria-pressed', active ? 'true' : 'false');
            button.disabled = !enabled || button.hasAttribute('data-mode-force-disabled');
        });
    ...
    (function bridgePhaseOneModules() {
        var app = window.HGCardGame || null;
        var utilsApi = app && app.core ? app.core.utils : null;
        var storageApi = app && app.core ? app.core.storage : null;
        var copyModelApi = app && app.data ? app.data.copyModel : null;
        var governanceApi = app && app.data ? app.data.governance : null;
        var rulesApi = app && app.data ? app.data.rules : null;
        var migrationsApi = app && app.data ? app.data.migrations : null;
        var collectionCoreApi = app && app.collection ? app.collection.core : null;
        var collectionFiltersApi = app && app.collection ? app.collection.filters : null;
        var collectionRenderApi = app && app.collection ? app.collection.render : null;
        var collectionImportExportApi = app && app.collection ? app.collection.importExport : null;
        var memoryCoreApi = app && app.memory ? app.memory.core : null;
        var memoryRenderApi = app && app.memory ? app.memory.render : null;
        var skillsApi = app && app.cards ? app.cards.skills : null;
        var evolveApi = app && app.cards ? app.cards.evolve : null;
        var improveApi = app && app.cards ? app.cards.improve : null;
        var recycleApi = app && app.cards ? app.cards.recycle : null;
        var teamsCoreApi = app && app.teams ? app.teams.core : null;
        var teamsLoadoutApi = app && app.teams ? app.teams.loadout : null;
        var combatStateApi = app && app.combat ? app.combat.state : null;
        var combatRulesApi = app && app.combat ? app.combat.rules : null;
        var combatAiApi = app && app.combat ? app.combat.ai : null;
        var combatAnimationsApi = app && app.combat ? app.combat.animations : null;
        var combatRenderApi = app && app.combat ? app.combat.render : null;
        var combatCoreApi = app && app.combat ? app.combat.core : null;
        var combatModesApi = app && app.combat ? app.combat.modes : null;
        var combatTrainingApi = combatModesApi && typeof combatModesApi.get === 'function'
            ? combatModesApi.get('training')
            : (combatModesApi ? combatModesApi.training : null);
        var combatDailyBossApi = combatModesApi && typeof combatModesApi.get === 'function'
            ? combatModesApi.get('daily-boss')
            : (combatModesApi ? combatModesApi.dailyBoss : null);

        if (utilsApi) {
            normalizeStorageScope = utilsApi.normalizeStorageScope || normalizeStorageScope;
            nowIso = utilsApi.nowIso || nowIso;
            clampInt = utilsApi.clampInt || clampInt;
            clampQuality = utilsApi.clampQuality || clampQuality;
            formatNumber = utilsApi.formatNumber || formatNumber;
            formatDate = utilsApi.formatDate || formatDate;
            escapeHtml = utilsApi.escapeHtml || escapeHtml;
            normalizeSearchText = utilsApi.normalizeSearchText || normalizeSearchText;
        }

        if (storageApi) {
            readJson = storageApi.readJson || readJson;
            writeJson = function (key, value) {
                var saved = storageApi.writeJson ? storageApi.writeJson(key, value) : false;
                if (!saved) {
                    setStatus(uiText('status.local_storage_error', 'No se pudo guardar en localStorage.'));
                }
                return saved;
            };
            readMigratedJson = storageApi.readMigratedJson || readMigratedJson;
            readText = storageApi.readText || readText;
            writeText = storageApi.writeText || writeText;
            readMigratedText = storageApi.readMigratedText || readMigratedText;
        }

        if (copyModelApi && typeof copyModelApi.configure === 'function') {
            copyModelApi.configure({
                clampInt: clampInt,
                normalizeRarity: normalizeRarity,
                normalizeSourceType: normalizeSourceType,
                rarityRank: rarityRank,
                getMoveLibrary: function () { return MOVE_LIBRARY; },
                getMoveLearnRules: function () { return MOVE_LEARN_RULES; },
                getRarityOrder: function () { return RARITY_ORDER; }
            });
            validCard = copyModelApi.validCard || validCard;
            normalizeMoveId = copyModelApi.normalizeMoveId || normalizeMoveId;
            cloneMoveDefinition = copyModelApi.cloneMoveDefinition || cloneMoveDefinition;
            normalizeCardMoves = copyModelApi.normalizeCardMoves || normalizeCardMoves;
            normalizeCopyMoveIds = copyModelApi.normalizeCopyMoveIds || normalizeCopyMoveIds;
            initialMoveIdsForCopy = copyModelApi.initialMoveIdsForCopy || initialMoveIdsForCopy;
            highestMoveCheckpoint = copyModelApi.highestMoveCheckpoint || highestMoveCheckpoint;
            addMoveIdsToCopy = copyModelApi.addMoveIdsToCopy || addMoveIdsToCopy;
            ensureCopyMovesForRarity = copyModelApi.ensureCopyMovesForRarity || ensureCopyMovesForRarity;
            normalizeCard = copyModelApi.normalizeCard || normalizeCard;
        }

        if (skillsApi && typeof skillsApi.configure === 'function') {
            skillsApi.configure({
                ...
                moveLibrary: function () { return MOVE_LIBRARY; },
                copyRarity: copyRarity,
                currentMnemones: currentMnemones,
                materialStock: materialStock,
                normalizeCurrency: normalizeCurrency,
                normalizeMaterialInventory: normalizeMaterialInventory,
                clampInt: clampInt,
                normalizeRarity: normalizeRarity,
                setStatus: setStatus,
                uiText: uiText,
                formatNumber: formatNumber,
                playSkillSound: playSkillSound,
                refreshCollectionViews: refreshCollectionViews,
                showCardModal: showCardModal,
                ownedCopiesForCard: ownedCopiesForCard,
                skillCostMultiplierByRarity: function () { return SKILL_COST_MULTIPLIER_BY_RARITY; },
                skillBaseMnemones: function () { return SKILL_BASE_MNEMONES; },
                skillMaterialKey: function () { return SKILL_MATERIAL_KEY; },
                upgradeMaterials: function () { return UPGRADE_MATERIALS; },
                skillSlotCount: function () { return SKILL_SLOT_COUNT; }
            });
            copyMoveDefinitions = skillsApi.copyMoveDefinitions || copyMoveDefinitions;
            copyHasLearnedMoves = skillsApi.copyHasLearnedMoves || copyHasLearnedMoves;
            skillCostMultiplier = skillsApi.skillCostMultiplier || skillCostMultiplier;
            skillMnemoneCost = skillsApi.skillMnemoneCost || skillMnemoneCost;
            skillMaterialCost = skillsApi.skillMaterialCost || skillMaterialCost;
            skillMaterialLabel = skillsApi.skillMaterialLabel || skillMaterialLabel;
            skillSlotState = skillsApi.skillSlotState || skillSlotState;
            availableSkillMoveIds = skillsApi.availableSkillMoveIds || availableSkillMoveIds;
            canAffordSkillRoll = skillsApi.canAffordSkillRoll || canAffordSkillRoll;
            skillShortageMessage = skillsApi.skillShortageMessage || skillShortageMessage;
            spendSkillRollCost = skillsApi.spendSkillRollCost || spendSkillRollCost;
            resetCopySkills = skillsApi.resetCopySkills || resetCopySkills;
            applySkillRoll = skillsApi.applySkillRoll || applySkillRoll;
        }

        if (governanceApi && typeof governanceApi.configure === 'function') {
            governanceApi.configure({
                clampInt: clampInt,
                clampQuality: clampQuality,
                normalizeRarity: normalizeRarity,
                getRarityStatRanges: function () { return RARITY_STAT_RANGES; }
            });
            copyRarity = governanceApi.copyRarity || copyRarity;
            rarityStatRange = governanceApi.rarityStatRange || rarityStatRange;
            statBoundsForRarity = governanceApi.statBoundsForRarity || statBoundsForRarity;
            statPercentInBounds = governanceApi.statPercentInBounds || statPercentInBounds;
            scaledStatForRarity = governanceApi.scaledStatForRarity || scaledStatForRarity;
            retuneCopyStatsForRarity = governanceApi.retuneCopyStatsForRarity || retuneCopyStatsForRarity;
            statForQuality = governanceApi.statForQuality || statForQuality;
            applyQualityToCopyStats = governanceApi.applyQualityToCopyStats || applyQualityToCopyStats;
            statsBelowRarityFloor = governanceApi.statsBelowRarityFloor || statsBelowRarityFloor;
            cardForCopy = governanceApi.cardForCopy || cardForCopy;
            totalStats = governanceApi.totalStats || totalStats;
            calculatedQualityScore = governanceApi.calculatedQualityScore || calculatedQualityScore;
            copyUpgradedFlag = governanceApi.copyUpgradedFlag || copyUpgradedFlag;
            qualityScore = governanceApi.qualityScore || qualityScore;
        }

        if (rulesApi && typeof rulesApi.configure === 'function') {
            rulesApi.configure({
                clampInt: clampInt,
                getCombatAdvancedRules: function () { return COMBAT_ADVANCED_RULES; }
            });
            hasOwn = rulesApi.hasOwn || hasOwn;
            assignObjectValue = rulesApi.assignObjectValue || assignObjectValue;
            assignArrayValue = rulesApi.assignArrayValue || assignArrayValue;
            normalizeRewardRangeConfig = rulesApi.normalizeRewardRangeConfig || normalizeRewardRangeConfig;
            normalizeDropConfigList = rulesApi.normalizeDropConfigList || normalizeDropConfigList;
            normalizeCombatDifficultyTable = rulesApi.normalizeCombatDifficultyTable || normalizeCombatDifficultyTable;
            normalizeCombatAdvancedRules = rulesApi.normalizeCombatAdvancedRules || normalizeCombatAdvancedRules;
        }

        if (migrationsApi && typeof migrationsApi.configure === 'function') {
            migrationsApi.configure({
                clampQuality: clampQuality,
                normalizeRarity: normalizeRarity,
        ...
        if (collectionCoreApi && typeof collectionCoreApi.configure === 'function') {
            collectionCoreApi.configure({
                getCollection: function () { return state.collection; },
                getCatalogById: function () { return state.catalogById; }
            });
            collectionGroups = collectionCoreApi.collectionGroups || collectionGroups;
            uniqueCollectionCount = collectionCoreApi.uniqueCollectionCount || uniqueCollectionCount;
        }

        if (collectionFiltersApi && typeof collectionFiltersApi.configure === 'function') {
            collectionFiltersApi.configure({
                getState: function () { return state; },
                getCatalog: function () { return state.catalog; },
                getTypeOrder: function () { return TYPE_ORDER; },
                ...
                copyRarity: copyRarity,
                typeLabel: typeLabel,
                typeLabels: function () { return TYPE_LABELS; },
                normalizeSearchText: normalizeSearchText
            });
            collectionFilterSets = collectionFiltersApi.collectionFilterSets || collectionFilterSets;
            cardPassesCollectionFilters = collectionFiltersApi.cardPassesCollectionFilters || cardPassesCollectionFilters;
            albumCategories = collectionFiltersApi.albumCategories || albumCategories;
            ensureAlbumCategory = collectionFiltersApi.ensureAlbumCategory || ensureAlbumCategory;
        }

        if (collectionRenderApi && typeof collectionRenderApi.configure === 'function') {
            collectionRenderApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                escapeHtml: escapeHtml,
                typeChipHtml: typeChipHtml,
                uiText: uiText,
                clampInt: clampInt,
                normalizePageSize: normalizePageSize,
                onPageChange: renderCollectionTable,
                onAlbumCategoryChange: renderCollectionTable
            });
            pageBounds = collectionRenderApi.pageBounds || pageBounds;
            renderPagination = collectionRenderApi.renderPagination || renderPagination;
            renderAlbumTabs = collectionRenderApi.renderAlbumTabs || renderAlbumTabs;
            renderCollectionTypeFilter = collectionRenderApi.renderCollectionTypeFilter || renderCollectionTypeFilter;
        }

        if (collectionImportExportApi && typeof collectionImportExportApi.configure === 'function') {
            collectionImportExportApi.configure({
                getState: function () {
                    state.RARITY_ORDER = RARITY_ORDER;
                    return state;
                },
                setCollection: function (collection) { state.collection = collection; },
                getCatalog: function () { return state.catalog; },
                getCatalogById: function () { return state.catalogById; },
                nowIso: nowIso,
                clampInt: clampInt,
                clampQuality: clampQuality,
                normalizeCurrency: normalizeCurrency,
                normalizePackInventory: normalizePackInventory,
                normalizeDailyShopPackPurchases: normalizeDailyShopPackPurchases,
                normalizeMaterialInventory: normalizeMaterialInventory,
                ...
                setStatus: setStatus,
                uiText: uiText,
                confirmGameAction: confirmGameAction,
                removeStorageKey: function (keyName) {
                    var keyValue = {
                        STORAGE_KEY: STORAGE_KEY,
                        LEGACY_STORAGE_KEY: LEGACY_STORAGE_KEY,
                        CARD_SHOP_STATE_KEY: CARD_SHOP_STATE_KEY,
                        LEGACY_FREE_REWARDS_KEY: LEGACY_FREE_REWARDS_KEY
                    }[keyName];
                    try { window.localStorage.removeItem(keyValue); } catch (e) {
                        if (keyName === 'STORAGE_KEY') { saveCollection(); }
                        if (keyName === 'CARD_SHOP_STATE_KEY') { saveShopState(); }
                    }
                }
            });
            validateCollection = collectionImportExportApi.validateCollection || validateCollection;
            exportCollection = collectionImportExportApi.exportCollection || exportCollection;
            importCollection = collectionImportExportApi.importCollection || importCollection;
            resetCollection = collectionImportExportApi.resetCollection || resetCollection;
        }

        if (memoryCoreApi && typeof memoryCoreApi.configure === 'function') {
            memoryCoreApi.configure({
                getState: function () { return state; },
                ...
                isCopyInCombatTeam: isCopyInCombatTeam,
                ownedCopiesForCard: ownedCopiesForCard,
                addMnemones: addMnemones,
                renderSummary: renderSummary,
                renderPackInventory: renderPackInventory,
                renderCollectionTable: renderCollectionTable,
                ...
                maxMnemones: function () { return MAX_MNEMONES; },
                startingMnemones: function () { return STARTING_MNEMONES; },
                workMnemonesPerUnique: function () { return WORK_MNEMONES_PER_UNIQUE; },
                workRarityBase: function () { return WORK_RARITY_BASE; },
                workMaxAssignments: function () { return WORK_MAX_ASSIGNMENTS; },
                workMinDurationMs: function () { return WORK_MIN_DURATION_MS; }
            });
            normalizeWorkAssignments = memoryCoreApi.normalizeWorkAssignments || normalizeWorkAssignments;
            normalizeWorkPendingRewards = memoryCoreApi.normalizeWorkPendingRewards || normalizeWorkPendingRewards;
            ensureWorkAssignments = memoryCoreApi.ensureWorkAssignments || ensureWorkAssignments;
            cleanWorkAssignments = memoryCoreApi.cleanWorkAssignments || cleanWorkAssignments;
            limitWorkAssignments = memoryCoreApi.limitWorkAssignments || limitWorkAssignments;
            workRatePerMinute = memoryCoreApi.workRatePerMinute || workRatePerMinute;
            workEntryFromAssignment = memoryCoreApi.workEntryFromAssignment || workEntryFromAssignment;
            enforceWorkRewardCap = memoryCoreApi.enforceWorkRewardCap || enforceWorkRewardCap;
            activeWorkEntries = memoryCoreApi.activeWorkEntries || activeWorkEntries;
            workCandidateEntries = memoryCoreApi.workCandidateEntries || workCandidateEntries;
            totalWorkClaimable = memoryCoreApi.totalWorkClaimable || totalWorkClaimable;
            workCanStop = memoryCoreApi.workCanStop || workCanStop;
            workRemainingLabel = memoryCoreApi.workRemainingLabel || workRemainingLabel;
            claimWorkRewards = memoryCoreApi.claimWorkRewards || claimWorkRewards;
            assignCopyToWork = memoryCoreApi.assignCopyToWork || assignCopyToWork;
            assignCopyToWorkById = memoryCoreApi.assignCopyToWorkById || assignCopyToWorkById;
            stopCopyWork = memoryCoreApi.stopCopyWork || stopCopyWork;
        }

        if (memoryRenderApi && typeof memoryRenderApi.configure === 'function') {
            memoryRenderApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                activeWorkEntries: activeWorkEntries,
                workCandidateEntries: memoryCoreApi && memoryCoreApi.workCandidateEntries ? memoryCoreApi.workCandidateEntries : workCandidateEntries,
                totalWorkClaimable: totalWorkClaimable,
                workCanStop: memoryCoreApi && memoryCoreApi.workCanStop ? memoryCoreApi.workCanStop : workCanStop,
                workRemainingLabel: memoryCoreApi && memoryCoreApi.workRemainingLabel ? memoryCoreApi.workRemainingLabel : workRemainingLabel,
                assignCopyToWorkById: assignCopyToWorkById,
                stopCopyWork: stopCopyWork,
                renderCard: renderCard,
                escapeHtml: escapeHtml,
                uiText: uiText,
                cardGameIconHtml: cardGameIconHtml,
                isMemoryContext: isMemoryContext,
                workMaxAssignments: function () { return WORK_MAX_ASSIGNMENTS; }
            });
            renderWorkBench = memoryRenderApi.renderWorkBench || renderWorkBench;
        }

        if (evolveApi && typeof evolveApi.configure === 'function') {
            evolveApi.configure({
                getState: function () { return state; },
                loadCollection: loadCollection,
                saveCollection: saveCollection,
                ...
                ownedCopiesForCard: ownedCopiesForCard,
                showCardModal: showCardModal,
                closeRarityUpgradeModal: closeRarityUpgradeModal,
                playDustSound: playDustSound,
                renderSummary: renderSummary,
                renderPackInventory: renderPackInventory,
                renderCollectionTable: renderCollectionTable,
                renderCombatSetup: renderCombatSetup,
                ...
                uiText: uiText,
                confirmGameAction: confirmGameAction,
                rarityUpgradeOrder: function () { return RARITY_UPGRADE_ORDER; },
                rarityUpgradeMultipliers: function () { return RARITY_UPGRADE_MULTIPLIERS; },
                upgradeCostByRarity: function () { return UPGRADE_COST_BY_RARITY; },
                rarityUpgradeMaterials: function () { return RARITY_UPGRADE_MATERIALS; },
                rarityUpgradeMinQuality: function () { return RARITY_UPGRADE_MIN_QUALITY; },
                rarityUpgradeRequired: function () { return RARITY_UPGRADE_REQUIRED; },
                rarityLabels: function () { return RARITY_LABELS; },
                copySortValue: copySortValue
            });
            nextRarity = evolveApi.nextRarity || nextRarity;
            upgradeMnemoneCost = evolveApi.upgradeMnemoneCost || upgradeMnemoneCost;
            rarityUpgradeMaterial = evolveApi.rarityUpgradeMaterial || rarityUpgradeMaterial;
            canPayUpgradeCost = evolveApi.canPayUpgradeCost || canPayUpgradeCost;
            spendUpgradeCost = evolveApi.spendUpgradeCost || spendUpgradeCost;
            rarityUpgradeCandidates = evolveApi.rarityUpgradeCandidates || rarityUpgradeCandidates;
            worseDuplicateCandidateMap = evolveApi.worseDuplicateCandidateMap || worseDuplicateCandidateMap;
            applyRarityUpgrade = evolveApi.applyRarityUpgrade || applyRarityUpgrade;
        }

        if (improveApi && typeof improveApi.configure === 'function') {
            improveApi.configure({
                getState: function () { return state; },
                loadCollection: loadCollection,
                clampQuality: clampQuality,
                copyRarity: copyRarity,
                ...
                renderCollectionTable: renderCollectionTable,
                renderCombatSetup: renderCombatSetup,
                closeQualityUpgradeModal: closeQualityUpgradeModal,
                showCardModal: showCardModal,
                ownedCopiesForCard: ownedCopiesForCard,
                setStatus: setStatus,
                uiText: uiText,
                formatNumber: formatNumber,
                qualityUpgradeMaxSlots: function () { return QUALITY_UPGRADE_MAX_SLOTS; }
            });
            qualityUpgradeContribution = improveApi.qualityUpgradeContribution || qualityUpgradeContribution;
            projectedQualityAfterSacrifices = improveApi.projectedQualityAfterSacrifices || projectedQualityAfterSacrifices;
            qualityUpgradeCandidates = improveApi.qualityUpgradeCandidates || qualityUpgradeCandidates;
            applyQualityUpgrade = improveApi.applyQualityUpgrade || applyQualityUpgrade;
        }

        if (recycleApi && typeof recycleApi.configure === 'function') {
            recycleApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                loadCollection: loadCollection,
                copyRarity: copyRarity,
                sortedCopies: sortedCopies,
                isCopyWorking: isCopyWorking,
                removeCopiesFromCombatTeams: removeCopiesFromCombatTeams,
                ...
                escapeHtml: escapeHtml,
                rarityLabels: function () { return RARITY_LABELS; },
                rarityOrder: function () { return RARITY_ORDER; },
                recycleValues: function () { return RECYCLE_VALUES; }
            });
            recycleValue = recycleApi.recycleValue || recycleValue;
            ownedCopiesForCard = recycleApi.ownedCopiesForCard || ownedCopiesForCard;
            favoriteCopyMap = recycleApi.favoriteCopyMap || favoriteCopyMap;
            isFavoriteCopy = recycleApi.isFavoriteCopy || isFavoriteCopy;
            toggleFavoriteCopy = recycleApi.toggleFavoriteCopy || toggleFavoriteCopy;
            recycleCopy = recycleApi.recycleCopy || recycleCopy;
            recycleDuplicateCopies = recycleApi.recycleDuplicateCopies || recycleDuplicateCopies;
            recycleAllCopies = recycleApi.recycleAllCopies || recycleAllCopies;
            bulkSellStats = recycleApi.bulkSellStats || bulkSellStats;
            renderBulkSellPreview = recycleApi.renderBulkSellPreview || renderBulkSellPreview;
            sellCardsByRarity = recycleApi.sellCardsByRarity || sellCardsByRarity;
        }

        if (teamsCoreApi && typeof teamsCoreApi.configure === 'function') {
            teamsCoreApi.configure({
                getState: function () { return state; },
                ...
                clampInt: clampInt,
                isCopyWorking: isCopyWorking,
                combatTeamsKey: function () { return COMBAT_TEAMS_KEY; },
                legacyCombatTeamsKey: function () { return LEGACY_COMBAT_TEAMS_KEY; },
                combatProfileKey: function () { return COMBAT_PROFILE_KEY; },
                legacyCombatProfileKey: function () { return LEGACY_COMBAT_PROFILE_KEY; }
            });
            createEmptyCombatTeams = teamsCoreApi.createEmptyCombatTeams || createEmptyCombatTeams;
            defaultCombatTeamName = teamsCoreApi.defaultCombatTeamName || defaultCombatTeamName;
            combatTeamDisplayName = teamsCoreApi.combatTeamDisplayName || combatTeamDisplayName;
            normalizeCombatTeams = teamsCoreApi.normalizeCombatTeams || normalizeCombatTeams;
            loadCombatTeams = teamsCoreApi.loadCombatTeams || loadCombatTeams;
            saveCombatTeams = teamsCoreApi.saveCombatTeams || saveCombatTeams;
            ownedCopyIdMap = teamsCoreApi.ownedCopyIdMap || ownedCopyIdMap;
            pruneDraftCombatTeam = teamsCoreApi.pruneDraftCombatTeam || pruneDraftCombatTeam;
            cleanCombatTeamsAgainstCollection = teamsCoreApi.cleanCombatTeamsAgainstCollection || cleanCombatTeamsAgainstCollection;
            removeCopiesFromCombatTeams = teamsCoreApi.removeCopiesFromCombatTeams || removeCopiesFromCombatTeams;
            normalizeCombatProfile = teamsCoreApi.normalizeCombatProfile || normalizeCombatProfile;
            loadCombatProfile = teamsCoreApi.loadCombatProfile || loadCombatProfile;
            saveCombatProfile = teamsCoreApi.saveCombatProfile || saveCombatProfile;
            combatPlayerName = teamsCoreApi.combatPlayerName || combatPlayerName;
            copyByInstanceId = teamsCoreApi.copyByInstanceId || copyByInstanceId;
            validDraftTeam = teamsCoreApi.validDraftTeam || validDraftTeam;
        }

        if (teamsLoadoutApi && typeof teamsLoadoutApi.configure === 'function') {
            teamsLoadoutApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                isCombatContext: isCombatContext,
                isCombatLoadoutVisible: isCombatLoadoutVisible,
                loadCollection: loadCollection,
                loadCombatTeams: loadCombatTeams,
                saveCombatTeams: saveCombatTeams,
                loadCombatProfile: loadCombatProfile,
                ...
                typeLabel: typeLabel,
                rarityLabels: function () { return RARITY_LABELS; },
                typeOrder: function () { return TYPE_ORDER; },
                uiText: uiText,
                escapeHtml: escapeHtml,
                setCombatMessage: setCombatMessage,
                updateCombatModeButtons: updateCombatModeButtons,
                loadDailyBossState: function () { return loadDailyBossState.apply(null, arguments); },
                dailyBossRewardClaimedToday: function () { return dailyBossRewardClaimedToday.apply(null, arguments); },
                renderDailyBossSummary: function () { return renderDailyBossSummary.apply(null, arguments); },
                renderCombatBattle: renderCombatBattle,
                normalizeCollectionRarity: normalizeCollectionRarity
            });
            combatEntryFromCopy = teamsLoadoutApi.combatEntryFromCopy || combatEntryFromCopy;
            combatStatPillHtml = teamsLoadoutApi.combatStatPillHtml || combatStatPillHtml;
            combatEntryStatsHtml = teamsLoadoutApi.combatEntryStatsHtml || combatEntryStatsHtml;
            combatOwnedEntries = teamsLoadoutApi.combatOwnedEntries || combatOwnedEntries;
            sortCombatEntries = teamsLoadoutApi.sortCombatEntries || sortCombatEntries;
            renderCombatTypeFilter = teamsLoadoutApi.renderCombatTypeFilter || renderCombatTypeFilter;
            renderCombatTeamSelect = teamsLoadoutApi.renderCombatTeamSelect || renderCombatTeamSelect;
            renderCombatTeamName = teamsLoadoutApi.renderCombatTeamName || renderCombatTeamName;
            renderCombatTeamPreview = teamsLoadoutApi.renderCombatTeamPreview || renderCombatTeamPreview;
            renderCombatProfile = teamsLoadoutApi.renderCombatProfile || renderCombatProfile;
            renderCombatTeamSlots = teamsLoadoutApi.renderCombatTeamSlots || renderCombatTeamSlots;
            renderCombatCardList = teamsLoadoutApi.renderCombatCardList || renderCombatCardList;
            autoBuildCombatTeam = teamsLoadoutApi.autoBuildCombatTeam || autoBuildCombatTeam;
            renderCombatSetup = teamsLoadoutApi.renderCombatSetup || renderCombatSetup;
            showCombatScreen = teamsLoadoutApi.showCombatScreen || showCombatScreen;
            saveDraftCombatTeam = teamsLoadoutApi.saveDraftCombatTeam || saveDraftCombatTeam;
            clearDraftCombatTeam = teamsLoadoutApi.clearDraftCombatTeam || clearDraftCombatTeam;
        }

        if (combatRulesApi && typeof combatRulesApi.configure === 'function') {
            combatRulesApi.configure({
                getState: function () { return state; },
                clampInt: clampInt,
                rarityOrder: function () { return RARITY_ORDER; },
                copyRarity: copyRarity,
                rollStat: rollStat,
                advancedRules: function () { return COMBAT_ADVANCED_RULES; },
                moveBuffMaxRatio: function () { return MOVE_BUFF_MAX_RATIO; },
                moveDebuffMinRatio: function () { return MOVE_DEBUFF_MIN_RATIO; },
                dailyBossStigmaticDamageMultiplier: function () { return DAILY_BOSS_STIGMATIC_DAMAGE_MULTIPLIER; }
            });
            rarityRank = combatRulesApi.rarityRank || rarityRank;
            rarityShieldCount = combatRulesApi.rarityShieldCount || rarityShieldCount;
            combatDifficultyRank = combatRulesApi.combatDifficultyRank || combatDifficultyRank;
            recalculateCombatStats = combatRulesApi.recalculateCombatStats || recalculateCombatStats;
            applyCombatModifier = combatRulesApi.applyCombatModifier || applyCombatModifier;
            clearCombatDebuffs = combatRulesApi.clearCombatDebuffs || clearCombatDebuffs;
            healCombatUnit = combatRulesApi.healCombatUnit || healCombatUnit;
            healDefendingUnit = combatRulesApi.healDefendingUnit || healDefendingUnit;
            breakCombatShields = combatRulesApi.breakCombatShields || breakCombatShields;
            effectiveDef = combatRulesApi.effectiveDef || effectiveDef;
            applyCombatDamage = combatRulesApi.applyCombatDamage || applyCombatDamage;
            combatDamageForAttackValue = combatRulesApi.combatDamageForAttackValue || combatDamageForAttackValue;
            combatDamage = combatRulesApi.combatDamage || combatDamage;
            combatMoveDamage = combatRulesApi.combatMoveDamage || combatMoveDamage;
            applyMoveEffect = combatRulesApi.applyMoveEffect || applyMoveEffect;
        }

        if (combatStateApi && typeof combatStateApi.configure === 'function') {
            combatStateApi.configure({
                getState: function () { return state; },
                clampInt: clampInt,
                normalizeCopyMoveIds: normalizeCopyMoveIds,
                cloneMoveDefinition: cloneMoveDefinition,
            ...
            normalizeCombatMoves = combatStateApi.normalizeCombatMoves || normalizeCombatMoves;
            createMoveState = combatStateApi.createMoveState || createMoveState;
            createCombatUnit = combatStateApi.createCombatUnit || createCombatUnit;
            activeCombatUnit = combatStateApi.activeCombatUnit || activeCombatUnit;
            livingCombatIndexes = combatStateApi.livingCombatIndexes || livingCombatIndexes;
            findUnitMove = combatStateApi.findUnitMove || findUnitMove;
            moveCooldownRemaining = combatStateApi.moveCooldownRemaining || moveCooldownRemaining;
            setMoveCooldown = combatStateApi.setMoveCooldown || setMoveCooldown;
            reduceMoveCooldowns = combatStateApi.reduceMoveCooldowns || reduceMoveCooldowns;
        }

        if (combatAiApi && typeof combatAiApi.configure === 'function') {
            combatAiApi.configure({
                getState: function () { return state; },
                uiText: uiText,
                clampInt: clampInt,
                ...
                combatDifficultyConfig: combatDifficultyConfig,
                combatDifficultyRank: combatDifficultyRank,
                combatEnemyTrainerName: combatEnemyTrainerName,
                playSkillSound: playSkillSound,
                pushCombatLog: pushCombatLog,
                advancedRules: function () { return COMBAT_ADVANCED_RULES; },
                dailyBossShieldBreakChance: function () { return DAILY_BOSS_SHIELD_BREAK_CHANCE; },
                markDailyBossCopyDefeated: markDailyBossCopyDefeated
            });
            currentCombatDifficultyKey = combatAiApi.currentCombatDifficultyKey || currentCombatDifficultyKey;
            currentCombatDifficultyRank = combatAiApi.currentCombatDifficultyRank || currentCombatDifficultyRank;
            currentCombatTurnNumber = combatAiApi.currentCombatTurnNumber || currentCombatTurnNumber;
            enemyUsableMoves = combatAiApi.enemyUsableMoves || enemyUsableMoves;
            enemyMoveScore = combatAiApi.enemyMoveScore || enemyMoveScore;
            pickEnemyMove = combatAiApi.pickEnemyMove || pickEnemyMove;
            enemySwitchCandidates = combatAiApi.enemySwitchCandidates || enemySwitchCandidates;
            enemySwitchScore = combatAiApi.enemySwitchScore || enemySwitchScore;
            pickEnemySwitch = combatAiApi.pickEnemySwitch || pickEnemySwitch;
            lockEnemySwitchState = combatAiApi.lockEnemySwitchState || lockEnemySwitchState;
            enemyTurn = combatAiApi.enemyTurn || enemyTurn;
        }

        if (combatAnimationsApi && typeof combatAnimationsApi.configure === 'function') {
            combatAnimationsApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                getRoot: function () { return root; },
                clampInt: clampInt,
                escapeHtml: escapeHtml,
                playCombatSound: playCombatSound,
                uiText: uiText,
                setCombatBusy: setCombatBusy,
                combatHitSoundDelayMs: function () { return COMBAT_HIT_SOUND_DELAY_MS; },
                combatDefendMs: function () { return COMBAT_DEFEND_MS; },
                combatDefeatMs: function () { return COMBAT_DEFEAT_MS; },
                combatEntryMs: function () { return COMBAT_ENTRY_MS; }
            });
            combatStand = combatAnimationsApi.combatStand || combatStand;
            combatScreenElement = combatAnimationsApi.combatScreenElement || combatScreenElement;
            restartCombatAnimation = combatAnimationsApi.restartCombatAnimation || restartCombatAnimation;
            showCombatDamage = combatAnimationsApi.showCombatDamage || showCombatDamage;
            spawnStandEffect = combatAnimationsApi.spawnStandEffect || spawnStandEffect;
            spawnAuraEffect = combatAnimationsApi.spawnAuraEffect || spawnAuraEffect;
            shakeCombatScreen = combatAnimationsApi.shakeCombatScreen || shakeCombatScreen;
            spawnCombatOrb = combatAnimationsApi.spawnCombatOrb || spawnCombatOrb;
            playMoveVfx = combatAnimationsApi.playMoveVfx || playMoveVfx;
            animateCombatAttack = combatAnimationsApi.animateCombatAttack || animateCombatAttack;
            animateCombatDefend = combatAnimationsApi.animateCombatDefend || animateCombatDefend;
            animateCombatDefeat = combatAnimationsApi.animateCombatDefeat || animateCombatDefeat;
            animateCombatEntry = combatAnimationsApi.animateCombatEntry || animateCombatEntry;
            removeCombatRivalIntro = combatAnimationsApi.removeCombatRivalIntro || removeCombatRivalIntro;
            playCombatRivalIntro = combatAnimationsApi.playCombatRivalIntro || playCombatRivalIntro;
            animateEnemyAction = combatAnimationsApi.animateEnemyAction || animateEnemyAction;
        }

        if (combatRenderApi && typeof combatRenderApi.configure === 'function') {
            combatRenderApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                getRoot: function () { return root; },
                isCombatContext: isCombatContext,
                ...
                renderCombatSetup: renderCombatSetup,
                renderCombatBattle: renderCombatBattle,
                startSelectedCombat: function () { return startSelectedCombat.apply(null, arguments); },
                showCombatScreen: showCombatScreen,
                switchPlayerCard: switchPlayerCard,
                renderDailyBossSummary: function () { return renderDailyBossSummary.apply(null, arguments); },
                dailyBossRewardSummary: function () { return dailyBossRewardSummary.apply(null, arguments); },
                applyCombatScreenBackground: applyCombatScreenBackground,
                combatScreenElement: combatScreenElement,
                animateEnemyAction: animateEnemyAction,
                combatEnemyTrainerName: combatEnemyTrainerName
            });
            setCombatMessage = combatRenderApi.setCombatMessage || setCombatMessage;
            pushCombatLog = combatRenderApi.pushCombatLog || pushCombatLog;
            showCombatCommandView = combatRenderApi.showCombatCommandView || showCombatCommandView;
            renderCombatMoveSlots = combatRenderApi.renderCombatMoveSlots || renderCombatMoveSlots;
            renderCombatActionState = combatRenderApi.renderCombatActionState || renderCombatActionState;
            renderCombatShields = combatRenderApi.renderCombatShields || renderCombatShields;
            renderCombatUnit = combatRenderApi.renderCombatUnit || renderCombatUnit;
            renderTrainingRivalProfile = combatRenderApi.renderTrainingRivalProfile || renderTrainingRivalProfile;
            renderCombatBench = combatRenderApi.renderCombatBench || renderCombatBench;
            renderCombatEndOverlay = combatRenderApi.renderCombatEndOverlay || renderCombatEndOverlay;
            renderCombatBattle = combatRenderApi.renderCombatBattle || renderCombatBattle;
            renderCombat = combatRenderApi.renderCombat || renderCombat;
        }

        if (combatModesApi && typeof combatModesApi.configure === 'function') {
            combatModesApi.configure({
                features: function () { return window.HG_CARD_GAME_FEATURES || {}; }
            });
            state.combatMode = normalizeCombatMode(state.combatMode, 'training');
        }

        if (combatTrainingApi && typeof combatTrainingApi.configure === 'function') {
            combatTrainingApi.configure({
                getState: function () { return state; },
                isCombatContext: isCombatContext,
                preloadCombatSounds: preloadCombatSounds,
                cleanCombatTeamsAgainstCollection: cleanCombatTeamsAgainstCollection,
                validDraftTeam: validDraftTeam,
                promptQuickCombatTeam: promptQuickCombatTeam,
                combatEntryFromCopy: function () { return combatEntryFromCopy.apply(null, arguments); },
                copyByInstanceId: copyByInstanceId,
                createCombatUnit: function () { return createCombatUnit.apply(null, arguments); },
                setCombatMessage: setCombatMessage,
                uiText: uiText,
                combatDifficultyConfig: combatDifficultyConfig,
                combatRewardMultiplier: combatRewardMultiplier,
                enemyDifficultyUsesMoves: enemyDifficultyUsesMoves,
                normalizeRarity: normalizeRarity,
                normalizeCopyMoveIds: normalizeCopyMoveIds,
                ...
                combatEnemyTrainerName: combatEnemyTrainerName,
                pushCombatLog: pushCombatLog,
                showCombatScreen: function () { return showCombatScreen.apply(null, arguments); },
                renderCombatBattle: function () { return renderCombatBattle.apply(null, arguments); },
                playCombatRivalIntro: playCombatRivalIntro,
                animateCombatEntry: animateCombatEntry,
                trainingRivalNames: function () { return TRAINING_RIVAL_NAMES; },
                trainingRivalTitles: function () { return TRAINING_RIVAL_TITLES; },
                rarityWeights: function () { return RARITY_WEIGHTS; },
                naturalRarityOrder: function () { return NATURAL_RARITY_ORDER; },
                loadCollection: loadCollection,
                addMnemones: addMnemones,
                saveCollection: saveCollection,
                renderSummary: renderSummary,
                rollStat: rollStat,
                trainingRewardTable: function () { return TRAINING_REWARD_TABLE; }
            });
            pickWeightedEnemyRarity = combatTrainingApi.pickWeightedEnemyRarity || pickWeightedEnemyRarity;
            pickEnemyCatalogCard = combatTrainingApi.pickEnemyCatalogCard || pickEnemyCatalogCard;
            ensureEnemyCopyMoves = combatTrainingApi.ensureEnemyCopyMoves || ensureEnemyCopyMoves;
            rivalPalette = combatTrainingApi.rivalPalette || rivalPalette;
            buildTrainingRivalSprite = combatTrainingApi.buildTrainingRivalSprite || buildTrainingRivalSprite;
            createTrainingRivalProfile = combatTrainingApi.createTrainingRivalProfile || createTrainingRivalProfile;
            createEnemyCard = combatTrainingApi.createEnemyCard || createEnemyCard;
            awardTrainingVictory = combatTrainingApi.awardTrainingVictory || awardTrainingVictory;
            startTrainingCombat = combatTrainingApi.startTrainingCombat || startTrainingCombat;
        }

        if (combatDailyBossApi && typeof combatDailyBossApi.configure === 'function') {
            combatDailyBossApi.configure({
                getState: function () { return state; },
                getEls: function () { return els; },
                readJson: readJson,
                writeJson: writeJson,
                dailyBossRewardKey: function () { return DAILY_BOSS_REWARD_KEY; },
                dailyBossStateKey: function () { return DAILY_BOSS_STATE_KEY; },
                dailyFreePackDate: dailyFreePackDate,
                normalizeTimestamp: normalizeTimestamp,
                createCardCopy: createCardCopy,
                rollStat: rollStat,
                clampInt: clampInt,
                ...
                renderSummary: renderSummary,
                renderCollectionTable: renderCollectionTable,
                renderCombatSetup: function () { return renderCombatSetup.apply(null, arguments); },
                renderCombatBattle: function () { return renderCombatBattle.apply(null, arguments); },
                setCombatMessage: setCombatMessage,
                setStatus: setStatus,
                uiText: uiText,
                ...
                dailyBossLootTable: function () { return DAILY_BOSS_LOOT_TABLE; },
                rarityOrder: function () { return RARITY_ORDER; },
                dailyBossCardReward: function () { return DAILY_BOSS_CARD_REWARD; },
                instanceId: instanceId,
                dailyBossHpMultiplierMin: function () { return DAILY_BOSS_HP_MULTIPLIER_MIN; },
                dailyBossHpMultiplierMax: function () { return DAILY_BOSS_HP_MULTIPLIER_MAX; }
            });
            dailyBossRewardState = combatDailyBossApi.dailyBossRewardState || dailyBossRewardState;
            dailyBossRewardClaimedToday = combatDailyBossApi.dailyBossRewardClaimedToday || dailyBossRewardClaimedToday;
            markDailyBossRewardClaimed = combatDailyBossApi.markDailyBossRewardClaimed || markDailyBossRewardClaimed;
            dailyBossActiveAttempt = combatDailyBossApi.dailyBossActiveAttempt || dailyBossActiveAttempt;
            pickDailyBossCard = combatDailyBossApi.pickDailyBossCard || pickDailyBossCard;
            createDailyBossState = combatDailyBossApi.createDailyBossState || createDailyBossState;
            normalizeDailyBossState = combatDailyBossApi.normalizeDailyBossState || normalizeDailyBossState;
            loadDailyBossState = combatDailyBossApi.loadDailyBossState || loadDailyBossState;
            saveDailyBossState = combatDailyBossApi.saveDailyBossState || saveDailyBossState;
            resetDailyBossState = combatDailyBossApi.resetDailyBossState || resetDailyBossState;
            dailyBossEntryFromState = combatDailyBossApi.dailyBossEntryFromState || dailyBossEntryFromState;
            updateDailyBossHp = combatDailyBossApi.updateDailyBossHp || updateDailyBossHp;
            startDailyBossAttempt = combatDailyBossApi.startDailyBossAttempt || startDailyBossAttempt;
            markDailyBossCopyDefeated = combatDailyBossApi.markDailyBossCopyDefeated || markDailyBossCopyDefeated;
            destroyDailyBossCopies = combatDailyBossApi.destroyDailyBossCopies || destroyDailyBossCopies;
            destroyDailyBossDefeatedCards = combatDailyBossApi.destroyDailyBossDefeatedCards || destroyDailyBossDefeatedCards;
            finishDailyBossAttempt = combatDailyBossApi.finishDailyBossAttempt || finishDailyBossAttempt;
            interruptDailyBossCombat = combatDailyBossApi.interruptDailyBossCombat || interruptDailyBossCombat;
            recoverAbandonedDailyBossAttempt = combatDailyBossApi.recoverAbandonedDailyBossAttempt || recoverAbandonedDailyBossAttempt;
            dailyBossRewardRarity = combatDailyBossApi.dailyBossRewardRarity || dailyBossRewardRarity;
            awardDailyBossLoot = combatDailyBossApi.awardDailyBossLoot || awardDailyBossLoot;
            dailyBossLootText = combatDailyBossApi.dailyBossLootText || dailyBossLootText;
            dailyBossRewardSummary = combatDailyBossApi.dailyBossRewardSummary || dailyBossRewardSummary;
            createDailyBoss = combatDailyBossApi.createDailyBoss || createDailyBoss;
            destroyDailyBossTeam = combatDailyBossApi.destroyDailyBossTeam || destroyDailyBossTeam;
            awardDailyBossVictory = combatDailyBossApi.awardDailyBossVictory || awardDailyBossVictory;
            appendDailyBossResetButton = combatDailyBossApi.appendDailyBossResetButton || appendDailyBossResetButton;
            renderDailyBossSummary = combatDailyBossApi.renderDailyBossSummary || renderDailyBossSummary;
            startDailyBossCombat = combatDailyBossApi.startDailyBossCombat || startDailyBossCombat;
        }

        if (combatCoreApi && typeof combatCoreApi.configure === 'function') {
            combatCoreApi.configure({
                getState: function () { return state; },
                uiText: uiText,
                clampInt: clampInt,
                activeCombatUnit: activeCombatUnit,
                ...
                combatTurnGapMs: function () { return COMBAT_TURN_GAP_MS; },
                combatAttackMs: function () { return COMBAT_ATTACK_MS; },
                combatDefendMs: function () { return COMBAT_DEFEND_MS; },
                combatEntryMs: function () { return COMBAT_ENTRY_MS; },
                combatDefeatMs: function () { return COMBAT_DEFEAT_MS; }
            });
            advanceDefeatedSide = combatCoreApi.advanceDefeatedSide || advanceDefeatedSide;
            resolveDefeatedSide = combatCoreApi.resolveDefeatedSide || resolveDefeatedSide;
            completePlayerTurn = combatCoreApi.completePlayerTurn || completePlayerTurn;
            finishEnemyAction = combatCoreApi.finishEnemyAction || finishEnemyAction;
            playerAttack = combatCoreApi.playerAttack || playerAttack;
            playerDefend = combatCoreApi.playerDefend || playerDefend;
            playerUseMove = combatCoreApi.playerUseMove || playerUseMove;
            switchPlayerCard = combatCoreApi.switchPlayerCard || switchPlayerCard;
            fleeCombat = combatCoreApi.fleeCombat || fleeCombat;
        }
    })();
    ...
    function spawnStandEffect(side, className, count) {
        var stand = combatStand(side);
        if (!stand) { return; }
        count = Math.max(1, Math.min(6, clampInt(count, 1)));
    ...
    function renderCombatShields(unit, node) {
        ...
        var signature = current + '/' + max;
        if (node.getAttribute('data-shields-signature') === signature) {
            return;
        }
        node.setAttribute('data-shields-signature', signature);
        node.innerHTML = '';
        for (var i = 0; i < max; i++) {
    ...
    function renderCombatUnit(unit, cardWrap, nameNode, hpNode, hpBar, shieldNode, atkNode, defNode) {
        if (nameNode) {
            if (unit) {
                setHtmlIfChanged(nameNode, combatCardNameHtml(unit.card, 'hg-combat-card-title--hud'));
            } else {
                setTextIfChanged(nameNode, '-');
            }
        }
        setTextIfChanged(hpNode, unit ? 'PS ' + unit.hp + ' / ' + unit.maxHp : 'PS 0 / 0');
        if (hpBar) {
            var hpWidth = unit ? Math.max(0, Math.min(100, (unit.hp / unit.maxHp) * 100)) + '%' : '0%';
            if (hpBar.style.width !== hpWidth) { hpBar.style.width = hpWidth; }
        }
        renderCombatShields(unit, shieldNode);
        setTextIfChanged(atkNode, unit ? String(unit.atk) : '0');
        setTextIfChanged(defNode, unit ? String(effectiveDef(unit)) : '0');
        if (cardWrap) {
    ...
    function renderCombatBattle() {
        ...
        renderCombatActionState();
        if (els.combatLog) {
            var logSignature = combat && combat.log.length ? combat.log.join('\n') : '';
            if (els.combatLog.getAttribute('data-log-signature') !== logSignature) {
                els.combatLog.setAttribute('data-log-signature', logSignature);
        ...
        if (els.combatBench && !els.combatBench.hidden) {
            renderCombatBench();
        }
    ...
    function renderCard(card, copy, options) {
        ...
        var flippable = !!copy && !options.combatUnit && !options.memoryCompact && !options.lightweight;
    ...
    function showPackReveal(cards, packKind) {
        ...
        if (!state.mobile) {
            ...
            cards.forEach(function (entry, index) {
                grid.appendChild(renderCard(entry.catalog, entry.instance, { reveal: true, lightweight: true, delay: index * 65 }));
            });
        }
    ...
    function renderMobileCardCarousel(cards, reveal) {
        ...
        function renderCurrent() {
            ...
            frame.innerHTML = '';
            frame.appendChild(renderCard(entry.catalog, entry.instance, { reveal: reveal, fresh: !reveal, lightweight: true, delay: 0 }));
    ...
    function startSelectedCombat() {
        var mode = normalizeCombatMode(state.combatMode, 'training');
        var registry = combatModesRegistry();
        state.combatMode = mode;
        if (registry && typeof registry.start === 'function') {
            return registry.start(mode);
        }
        return mode === 'daily-boss' ? startDailyBossCombat() : startTrainingCombat();
    }
    ...
    function bindCollectionControls() {
        ...
        els.combatModeButtons.forEach(function (button) {
            button.addEventListener('click', function () {
                var nextMode = button.getAttribute('data-combat-mode') || 'training';
                var meta = combatModeMeta(nextMode);
                if (state.combat && !state.combat.over) {
                    setCombatMessage(uiText('combat.finish_before_mode', 'Termina el combate actual antes de cambiar de modo.'));
                    return;
                }
                if (!isCombatModeEnabled(nextMode)) {
                    setCombatMessage(uiText('combat.mode_unavailable', 'Este modo todavía no está disponible.'));
                    return;
                }
                if (state.combat && state.combat.over) {
                    state.combat = null;
                }
                state.combatMode = normalizeCombatMode(nextMode, 'training');
                updateCombatModeButtons();
                setCombatMessage(meta && meta.selectedMessage
                    ? meta.selectedMessage
                    : (state.combatMode === 'daily-boss'
    ...
    function startShopStateTimer() {
        if (shopApi && shopApi.stateTimer && typeof shopApi.stateTimer.start === 'function') {
            shopApi.stateTimer.start({
                state: state,
                syncShopState: syncShopState,
                renderDailyCounter: renderDailyCounter,
                renderShop: renderShop,
                shouldRenderShop: isShopVisible,
                renderWorkBench: renderWorkBench,
                intervalMs: 30000
            });
            return;
    ...
    function bindRootClick(selector, handler) {
        if (eventsApi && typeof eventsApi.onRootClick === 'function') {
            eventsApi.onRootClick(root, selector, handler);
            return;
        }
    ...
    function isShopVisible() {
        if (state.mobile) {
            return els.mobilePanels.some(function (panel) {
                return panel.getAttribute('data-mobile-panel') === 'shop' && panel.classList.contains('is-active');
            });
        }
        return state.view === 'gacha' && window.location.hash === '#shop';
    }

    function scheduleShopRender() {
        if (!isShopVisible()) { return; }
        if (shopRenderFrame) { return; }
        var schedule = window.requestAnimationFrame || function (callback) { return window.setTimeout(callback, 0); };
        shopRenderFrame = schedule(function () {
            shopRenderFrame = 0;
            renderShop();
        });
    }

    function createPacksContext() {
        return {
            state: state,
            els: els,
            PACK_KINDS: PACK_KINDS,
            PACK_SIZE: PACK_SIZE,
            MAX_PACK_STOCK: MAX_PACK_STOCK,
            clampInt: clampInt,
            uiText: uiText,
            loadCollection: loadCollection,
            saveCollection: saveCollection,
            normalizePackInventory: normalizePackInventory,
            renderSummary: renderSummary,
            renderShop: scheduleShopRender,
            ensurePackLayout: ensurePackLayout,
            applyPackSkin: applyPackSkin,
            setStatus: setStatus,
            packLabel: packLabel,
            pickRarity: pickRarity,
            pickCardByRarity: pickCardByRarity,
            createCardCopy: createCardCopy,
            cardAllowedForPack: cardAllowedForPack,
            playPackOpenSound: playPackOpenSound,
            renderPackResults: renderPackResults,
            showPackReveal: showPackReveal
        };
    }

    function getPacksDomain() {
        if (!packsDomain && packsApi && packsApi.domain && typeof packsApi.domain.create === 'function') {
            packsDomain = packsApi.domain.create(createPacksContext());
        }
        return packsDomain;
    }
    ...
    function createShopContext() {
        return {
            ...
            CARD_SHOP_STATE_KEY: CARD_SHOP_STATE_KEY,
            LEGACY_FREE_REWARDS_KEY: LEGACY_FREE_REWARDS_KEY,
            DAILY_GIFT_MATERIAL_KEYS: DAILY_GIFT_MATERIAL_KEYS,
            DAILY_FREE_PACK_CAP: DAILY_FREE_PACK_CAP,
            SHOP_PACK_KINDS: SHOP_PACK_KINDS,
            SHOP_QUANTITIES: SHOP_QUANTITIES,
            FREE_SHOP_QUANTITIES: FREE_SHOP_QUANTITIES,
            UPGRADE_MATERIALS: UPGRADE_MATERIALS,
            PACK_KINDS: PACK_KINDS,
            PACK_PRICES: PACK_PRICES,
            PACK_CONTENTS: PACK_CONTENTS,
            PACK_LABELS: PACK_LABELS,
            PACK_DAILY_CAPS: PACK_DAILY_CAPS,
            DAILY_MAGIC_PACK_CAP: DAILY_MAGIC_PACK_CAP,
            DAILY_SHOP_PACK_CAP: DAILY_SHOP_PACK_CAP,
            MAX_PACK_STOCK: MAX_PACK_STOCK,
            ...
            readMigratedJson: readMigratedJson,
            writeJson: writeJson,
            rulesReady: function () { return !!state.rulesCatalog && gameRulesPayloadReady(); },
            loadCollection: loadCollection,
            saveCollection: saveCollection,
            ensureShopLayout: ensureShopLayout,
            materialIconHtml: materialIconHtml,
            materialStock: materialStock,
            addMaterial: addMaterial,
            currentMnemones: currentMnemones,
            addMnemones: addMnemones,
    ...
    function getShopDomain() {
        if (!shopDomain && shopApi && shopApi.domain && typeof shopApi.domain.create === 'function') {
            shopDomain = shopApi.domain.create(createShopContext());
        }
        return shopDomain;
    }

    function bindPackControls() {
        if (packsApi && packsApi.controls && typeof packsApi.controls.bind === 'function') {
            packsApi.controls.bind({
                root: root,
                events: eventsApi,
                openPack: openPack,
        ...
        bindRootClick('[data-pack-kind]', function (event, packButton) {
            event.preventDefault();
            openPack(packButton.getAttribute('data-pack-kind') || 'standard');
        });
        bindRootClick('[data-pack-open-all]', function (event) {
            event.preventDefault();
            openAllPacks();
        });
    }
    ...
    function bindShopControls() {
        if (shopApi && shopApi.controls && typeof shopApi.controls.bind === 'function') {
            shopApi.controls.bind({
                root: root,
                events: eventsApi,
                buyPack: buyPack,
        ...
        bindRootClick('[data-shop-buy-pack]', function (event, button) {
            event.preventDefault();
        ...
        bindRootClick('[data-shop-buy-material]', function (event, button) {
            event.preventDefault();
            buyMaterial(button.getAttribute('data-shop-buy-material') || '', button.getAttribute('data-shop-buy-amount') || 1);
        });
        bindRootClick('[data-shop-buy-exchange-remorias]', function (event, button) {
            event.preventDefault();
            buyRemoriaExchange(button.getAttribute('data-shop-buy-exchange-remorias') || 0);
        });
        bindRootClick('[data-shop-claim-daily-gift]', function (event, button) {
            event.preventDefault();
    ...
    function bindRuntimeHashChange() {
        if (eventsApi && typeof eventsApi.onHashChange === 'function') {
            eventsApi.onHashChange(updateDesktopHashPanels);
            return;
        }
        window.addEventListener('hashchange', updateDesktopHashPanels);
    }

    if (appRuntime && typeof appRuntime.runLegacyRuntime === 'function') {
        appRuntime.runLegacyRuntime({
            loadCollectionViewPrefs: loadCollectionViewPrefs,
            decorateIconNavigation: decorateIconNavigation,
            updateDesktopHashPanels: updateDesktopHashPanels,
            bindHashChange: bindRuntimeHashChange,
            bindEvents: bindEvents,
            loadGameRules: loadGameRules,
            loadCollection: loadCollection,
            renderAfterCollectionLoad: renderAfterCollectionLoad,
            startShopStateTimer: startShopStateTimer,
