<?php
setMetaFromPage("Totems | Heaven's Gate", "Listado de totems por categoria.", null, 'website');
...
$consulta = "SELECT id, pretty_id, name, coste FROM dim_totems WHERE tipo = ? ORDER BY coste";
$stmt = $link->prepare($consulta);
$stmt->bind_param('s', $routeParam);
$stmt->execute();
$result = $stmt->get_result();
...
while ($row = $result->fetch_assoc()) {
    echo "
        <a href='" . htmlspecialchars(pretty_url($link, 'dim_totems', '/powers/totem', (int)$row["id"])) . "'>
            <div class='renglon2col'>
                <div class='renglon2colIz'>
                    <img class='valign' src='img/ui/powers/totem.gif'> " . htmlspecialchars($row["name"]) . "
                </div>
