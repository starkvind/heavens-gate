...
<?php else: ?>
    <?php foreach ($origins as $origin): ?>
        ...
        <?php echo "<h3 class='hg-powers-origin-toggle' data-powers-toggle data-target='" . h($fieldsetId) . "'>" . h($origin) . "</h3>"; ?>
        <fieldset class="hg-powers-group-list">
            <?php echo "<div id='" . h($fieldsetId) . "' class='hg-powers-origin-content'>"; ?>
                ...
                <a href="<?= h($href) ?>">
                    <div class="hg-powers-list-card">
                        <div class="hg-powers-list-card__main">
                            <span class="hg-powers-item-cell"><span class="hg-powers-item-icon"><img class="hg-powers-item-thumb" src="<?= h($img) ?>" alt="<?= h($name) ?>"></span><?= h($name) ?></span>
                        </div>
                        <div class="hg-powers-list-card__meta"><?= h($row["cost"]) ?></div>
                    </div>
                </a>
            <?php endforeach; ?>
            </div>
...
document.addEventListener('DOMContentLoaded', function(){
    var toggles = document.querySelectorAll('[data-powers-toggle]');
