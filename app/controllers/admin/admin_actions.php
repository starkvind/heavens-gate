...
<div class="adm-actions-table adm-table-scroll adm-sticky-actions" tabindex="0" aria-label="Tabla de acciones"><table class="table adm-wide-table"><thead><tr><th>Imagen</th><th class="adm-col-name">Acción</th><th>Categoría</th><th class="adm-col-text">Tirada</th><th>Dificultad</th><th class="adm-col-text">Descripción</th><th class="adm-th-actions">Opciones</th></tr></thead><tbody id="actionsTbody">
...
<tr data-search="<?= admin_actions_h($search) ?>" data-category="<?= admin_actions_h(trim((string)$action['category'])) ?>"><td><?php if (!empty($action['image_url'])): ?><img class="adm-action-image" src="<?= admin_actions_h($action['image_url']) ?>" alt=""><?php else: ?><span class="adm-action-empty">Sin imagen</span><?php endif; ?></td><td><?= admin_actions_h($action['name']) ?></td><td><?= admin_actions_h($action['category']) ?></td><td><?= admin_actions_h($action['attribute_name']) ?> + <?= admin_actions_h($action['skill_name']) ?></td><td><?= $action['difficulty_mode'] === 'fixed' ? 'Fija: ' . (int)$action['fixed_difficulty'] : ((int)$action['min_difficulty'] . '–' . (int)$action['max_difficulty'] . ' (sug. ' . (int)$action['suggested_difficulty'] . ')') ?></td><td class="adm-action-desc" title="<?= admin_actions_h(strip_tags((string)$action['text'])) ?>"><?= admin_actions_h(strip_tags((string)$action['text'])) ?></td><td class="adm-cell-actions"><div class="adm-actions-inline"><button class="btn" type="button" data-action-edit="<?= (int)$action['id'] ?>">Editar</button> <button class="btn btn-red" type="button" data-action-delete="<?= (int)$action['id'] ?>">Borrar</button></div></td></tr>
<?php endforeach; ?>
