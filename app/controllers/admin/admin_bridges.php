...
include_once(__DIR__ . '/../../domains/organizations/admin_bridges.php');
include_once(__DIR__ . '/../../helpers/admin_ajax.php');
...
// Cargar estado
// ============================
$state = bridges_load_state($link,$T_CHAR_GROUP,$T_CHAR_CLAN,$T_CLAN_GROUP,$excludeChronicles);
$opts_clanes = $state['opts_clanes'];
$opts_manadas = $state['opts_manadas'];
$charGroupHasId = $state['charGroupHasId'];
$charClanHasId = $state['charClanHasId'];
$clanGroupHasId = $state['clanGroupHasId'];
$chars = $state['chars'];
foreach ($chars as &$row) {
  $row['avatar_url'] = hg_character_avatar_url((string)($row['image_url'] ?? ''), (string)($row['gender'] ?? ''));
}
unset($row);
