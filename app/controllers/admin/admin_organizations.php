...
include_once(__DIR__ . '/../../domains/organizations/admin.php');
...
if ($isAjaxRequest) {
    ...
    if ($action === 'update_org') {
        ...
        $result = hg_aorg_update_org($link,$orgId,$name,$sortOrder,$totemId,$color,$isNpc,$description);
        if (empty($result['ok'])) {
    ...
    if ($action === 'save_department') {
        ...
        $result = hg_aorg_save_department($link,$orgId,$departmentId,$parentIdOrNull,$pretty,$name,$type,$level,$color,$description,$sortOrder,$isActive);
        if (empty($result['ok'])) {
            hg_admin_json_error((string)$result['message'], 500, ['db'=>(string)($result['error'] ?? '')]);
        }
        $newId = (int)$result['id'];
    ...
    if ($action === 'save_position' || $action === 'assign_position') {
        ...
        if ($action === 'assign_position') {
            ...
            $result = hg_aorg_assign_position_character($link,$orgId,$positionId,$characterIdOrNull,$isActive);
            if (empty($result['ok'])) {
                hg_admin_json_error((string)$result['message'], 500, ['db'=>(string)($result['error'] ?? '')]);
            }
        ...
        $result = hg_aorg_save_position(
            $link,$orgId,$positionId,$characterIdOrNull,$departmentId,$parentIdOrNull,$level,
            $positionName,$positionCode,$scopeLabel,$responsibility,$isHead,$isPrimary,$isActive,$sortOrder
        );
        if (empty($result['ok'])) {
            hg_admin_json_error((string)$result['message'], 500, ['db'=>(string)($result['error'] ?? '')]);
        }
