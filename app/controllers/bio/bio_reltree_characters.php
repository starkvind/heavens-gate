<?php
setMetaFromPage(
    "Nebulosa de personajes | Heaven's Gate",
...
if (!$link) {
    ...
    hg_public_render_error(
        'Mapa no disponible',
        'No se pudo cargar el mapa de relaciones entre personajes en este momento.'
    );
    return;
}

if (!function_exists('hg_bio_reltree_sanitize_int_csv')) {
    function hg_bio_reltree_sanitize_int_csv($csv): string
    {
        $csv = (string)$csv;
        if (trim($csv) === '') {
            return '';
...
<div class="bioTextData">
    <fieldset class="bioSeccion">
        ...
        <div style="float: right;">
            ...
            <button class="boton2" onclick="location.href='/relationship-map/organizations'">Cambiar vista</button>
...
<?php foreach ($characters as $character): ?>
    <?php
        $hasImage = !empty($character['image_url']);
        $status = (string)($character['status'] ?? '');
        $isDead = ($status === 'Cadaver' || $status === 'Cadáver');
        $label = (string)($character['name'] ?? '') . ($isDead ? ' +' : '');
        $nodeColor = $isDead ? [
            'background' => '#888',
            'border' => '#555',
...
<?php if ($hasImage): ?>
        image: <?= json_encode("../" . $character['image_url'], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
<?php endif; ?>
        size: 25,
...
<?php foreach ($relations as $relation): ?>
    <?php
        ...
        $importance = isset($relation['importance']) ? (int)$relation['importance'] : 1;
        if ($importance < 1) {
            $importance = 1;
        }

        $arrows = (string)($relation['arrows'] ?? '');
        if ($arrows === '') {
            $arrows = 'to';
        }
    ?>
    {
        from: <?= (int)($relation['source_id'] ?? 0) ?>,
        to: <?= (int)($relation['target_id'] ?? 0) ?>,
        label: <?= json_encode((string)($relation['relation_type'] ?? ''), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
        width: <?= $importance ?>,
...
network.on("doubleClick", function (params) {
    if (params.nodes.length === 1) {
        window.open(`/characters/${params.nodes[0]}`, '_blank');
    }
});
