<?php
	...
	*/
	...
	$idGetData = isset($_GET['b']) ? (int)$_GET['b'] : 0; // Cogemos datos del GET "b"
	$characterId = $idGetData;
	if ($idGetData <= 0) {
	...
	mysqli_stmt_bind_param($stmtMain, 'i', $idGetData);
	...
	if ($NFilas > 0) { // Comenzamos chequeo de datos. Si no tenemos nada, mandamos un mensaje de error.
		...
		// Datos básicos del personaje
			$bioId		 = $dataResult["id"]; 			// ID del personaje. Aunque la tengamos en el get, mejor así.
	...
	echo "<div class='bioBody'>"; // CUERPO PRINCIPAL DE LA FICHA DE INFORMACION
		...
		mostrarTarjetaBSO($link, 'personaje', $idGetData);
		// ================================================================== //
		...
		<?php
			...
			$html = "<pre style='background:#111; border:1px solid #444; color:#0f0; font-family:monospace; padding:0.5em; border-radius:6px; overflow:auto;'><code>[hg_avatar=" . h($idGetData) . "," . h($bioTxtColor) . "]Mensaje de " . h($bioName) . "[/hg_avatar]</code></pre>";
			echo $html;
		?>
			</fieldset>
		</div>
