...
$consultaPag = "SELECT COUNT(*) as total FROM dim_archetypes";
...
$consulta = "SELECT id, name FROM dim_archetypes ORDER BY name ASC";
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        echo "
            <a 
                href='" . htmlspecialchars(pretty_url($link, 'dim_archetypes', '/rules/archetypes', (int)$row["id"])) . "' 
