<?php
require_once __DIR__ . '/../../domains/rules/queries.php';

setMetaFromPage("Reglamento | Heaven's Gate", "Resumen y acceso al reglamento del juego.", null, 'website');

$pageSect = 'Reglamento';

if (function_exists('hg_page_register_stylesheet')) {
    hg_page_register_stylesheet('/assets/css/hg-archive.css');
    hg_page_register_stylesheet('/assets/css/hg-docs.css');
}

if (!function_exists('hg_rules_home_h')) {
    function hg_rules_home_h($value): string
    {
        return htmlspecialchars((string)$value, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    }
}

$counts = isset($link) && ($link instanceof mysqli) ? hg_rules_fetch_home_counts($link) : [];
$rulesTypes = [
    ['key' => 'traits', 'name' => 'Rasgos', 'href' => '/rules/traits', 'desc' => 'Rasgos numéricos, como Atributos y Habilidades.', 'image' => '/img/og/og_image_bio.webp'],
    ['key' => 'merits', 'name' => 'Méritos y Defectos', 'href' => '/rules/merits-flaws', 'desc' => 'Rasgos que definen ventajas y debilidades del personaje.', 'image' => '/img/og/og_image_power.webp'],
    ['key' => 'conditions', 'name' => 'Condiciones', 'href' => '/rules/conditions', 'desc' => 'Deformidades, heridas de guerra y trastornos mentales que afectan a los personajes.', 'image' => '/img/og/og_image_monster.webp'],
    ['key' => 'actions', 'name' => 'Acciones', 'href' => '/rules/actions', 'desc' => 'Tiradas básicas que combinan un Atributo, una Habilidad y una dificultad.', 'image' => '/img/og/og_image_temp.webp'],
    ['key' => 'archetypes', 'name' => 'Personalidades', 'href' => '/rules/archetypes', 'desc' => 'Arquetipos de personalidad que ayudan a interpretar el personaje.', 'image' => '/img/og/og_image.webp'],
    ['key' => 'maneuvers', 'name' => 'Maniobras de pelea', 'href' => '/rules/maneuvers', 'desc' => 'Técnicas de combate que van más allá de la pelea simple.', 'image' => '/img/og/og_image_power.webp'],
];
foreach ($rulesTypes as &$rule) {
    $rule['count'] = $counts[$rule['key']] ?? null;
}
unset($rule);
?>

<main class="chron-detail rules-chron-home" aria-labelledby="rules-home-title">
    <section class="chron-box">
        <div class="chron-box-head">
            <h2 id="rules-home-title">Reglamento</h2>
            <p>Material de consulta para interpretar personajes, resolver acciones y preparar el juego.</p>
        </div>
        <div class="chron-grid" aria-label="Secciones del reglamento">
            <?php foreach ($rulesTypes as $ruleIndex => $rule): ?>
                <a class="chron-card" href="<?= hg_rules_home_h($rule['href']) ?>">
                    <img src="<?= hg_rules_home_h($rule['image']) ?>" alt="" loading="<?= $ruleIndex < 2 ? 'eager' : 'lazy' ?>" decoding="async">
                    <div class="chron-card-body">
                        <h3><?= hg_rules_home_h($rule['name']) ?></h3>
                        <p><?= hg_rules_home_h($rule['desc']) ?></p>
                        <div class="chron-card-meta">
                            <?php if ($rule['count'] !== null): ?><span><?= number_format((int)$rule['count'], 0, ',', '.') ?> elementos</span><?php endif; ?>
                            <span>Consultar &rarr;</span>
                        </div>
                    </div>
                </a>
            <?php endforeach; ?>
        </div>
    </section>
</main>